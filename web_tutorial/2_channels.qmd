...
**queue channels** and **value channels. Queue channels** are expendable, they can only be used once - like when you pipe data in bash. Once used, they vanish. Most channels in nextflow are queue channels. Value channels are long-lived and can be re-used. They usually hold constant values (or absolute paths).
...
This creates a channel which is a tuple of two elements. First one is the sample ID, and second one is a list of two elements, which are paths to file_1.fq and file_2.fq.
